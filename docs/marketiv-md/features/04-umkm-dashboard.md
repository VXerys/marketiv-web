...
## 1. Purpose
Mendefinisikan dashboard utama UMKM untuk ringkasan campaign, escrow, order Rate Card, transaksi, dan actionable next step.
...
## 4. User Stories
1. Sebagai UMKM, saya ingin melihat ringkasan campaign dan dana agar bisa mengambil keputusan cepat.
2. Sebagai UMKM, saya ingin melihat CTA utama untuk membuat campaign baru.
3. Sebagai UMKM, saya ingin mengetahui submission/order yang membutuhkan perhatian.

## 5. Main Flow
1. UMKM login.
2. Dashboard memuat metric utama.
3. Dashboard menampilkan campaign terbaru.
4. Dashboard menampilkan transaksi dan order aktif.
5. User memilih CTA lanjutan.
...
## 8. Validation Rules
- User harus role UMKM.
- Metric hanya berdasarkan data milik UMKM.
- Tanggal dan nominal harus diformat lokal Indonesia.
- Data error tidak boleh ditampilkan sebagai nol tanpa penanda.

## 9. Backend Responsibilities
- Menyediakan agregasi metric atau query efisien.
- Memastikan UMKM hanya melihat data miliknya.
- Menyediakan summary transaction/order/campaign.
- Menghasilkan notification bila ada action required.

## 10. Frontend Responsibilities
- Membuat layout metric cards.
- Menampilkan skeleton saat loading.
- Menampilkan CTA Buat Campaign Baru.
- Menampilkan status badge dan action required secara jelas.
...
## 12. Edge Cases
- Belum ada campaign.
- Payment pending lebih dari batas waktu.
- Campaign aktif tapi kuota penuh.
- Submission pending menumpuk.
...
- [ ] Dashboard hanya bisa dibuka UMKM.
- [ ] Metric sesuai data milik user.
- [ ] Empty state mengarahkan ke Buat Campaign.
- [ ] Semua status badge konsisten.
